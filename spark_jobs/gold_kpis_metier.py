...
# Jointure activite + etablissements
kpi_consultation_etablissement = df_activite \
    .join(df_etablissements.select("_sk", "raison_sociale_site", "commune"), 
          df_activite["_sk"] == df_etablissements["_sk"], "left") \
...
print(f"   💾 {kpi_consultation_etablissement.count()} lignes (consultations par établissement)")

# ===== KPI 2: Taux de consultation par professionnel =====
print("\n📊 KPI 2: TAUX DE CONSULTATION PAR PROFESSIONNEL")

kpi_consultation_professionnel = df_activite \
    .join(df_professionnels.select("_sk", "code_profession", "code_categorie_professionnelle"),
          df_activite["_sk"] == df_professionnels["_sk"], "left") \
    .groupBy("code_profession", "code_categorie_professionnelle") \
    .agg(
        F.count("*").alias("nombre_consultations"),
...
print(f"   💾 {kpi_consultation_professionnel.count()} lignes (consultations par professionnel)")
...
print(f"   💾 {kpi_hospitalisation_sexe_age.count()} lignes (hospitalisation par sexe/âge)")
...
print(f"   💾 {kpi_deces_region_2019.count()} lignes (décès par région 2019)")
